...
public class CPHInline
{
  public bool Execute()
  {
    ...
    //parameter 1
    string filePath = @"..\references\hc.txt";
    string[] lines = File.ReadAllLines(filePath);
    string reference = args["input0"].ToString();
    string arg_userId = args["userId"].ToString();
    string textname = "Heidelberg Catechism";
    string commandname = "!hc";
    int hc_counter = CPH.GetTwitchUserVarById<int>(arg_userId, "hc_counter",true);
    ...
    if (referenceMap.TryGetValue(reference, out int map_var)){
      //Console.WriteLine($"The wcf_counter for reference '{reference}' is: {hc_counter}");
      //CPH.SendMessage(map_var.ToString());
      hc_counter = map_var;
    }
    else if (reference == "next"){
      if (hc_counter != lines.Length - 1)
      hc_counter++;
    }
    ...
    //CPH.SendMessage("Lines length: " + lines.Length.ToString() + " hc_counter: "+ hc_counter);
    //lines[wcf_counter].Replace("|", "")
    if (lines[hc_counter].Length >= 500)
    {
      int chunkSize = 500;
      int stringLength = lines[hc_counter].Length;
      for (int i = 0; i < stringLength ; i += chunkSize)
      {
        if (i + chunkSize > stringLength) chunkSize = stringLength  - i;
        CPH.SendMessage(lines[hc_counter].Substring(i, chunkSize),false);
    ...
    {
      CPH.SendMessage(lines[hc_counter], false);
    }
